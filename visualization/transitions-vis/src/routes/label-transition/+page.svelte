<script lang="ts">
  ...
  async function loadAllData() {
    ...
    error = null;
    try {
      const data = await d3.csv<DSVRowExtended>("/data/label-transition/scholawrite_train.xls");
      allRecords = data
        .map((d) => ({
  ...
  function drawTimeAxis(container: d3.Selection<SVGGElement, unknown, null, undefined>) {
    const maxStep = records.length;
    const innerWidth = config.width - config.margin.left - config.margin.right;
    const chartHeight = config.timeAxisHeight - 20;
    const offset = 40;
    ...
    const stepCounts: { step: number; PLANNING: number; IMPLEMENTATION: number; REVISION: number }[] = [];
    let running = { PLANNING: 0, IMPLEMENTATION: 0, REVISION: 0 };

    records.forEach((d, i) => {
      if (d.high_level in running) {
        running[d.high_level as keyof typeof running]++;
      }
      stepCounts.push({
        step: i + 1,
    ...
    const yScale = d3
      .scaleLinear()
      .domain([0, d3.max(stepCounts, d => Math.max(d.PLANNING, d.IMPLEMENTATION, d.REVISION)) || 1])
      .range([
        config.height - config.margin.bottom + chartHeight + offset,
        config.height - config.margin.bottom + offset
      ]);

    container.selectAll(".time-axis").remove();
    container.selectAll(".brush").remove();
    container.selectAll(".brush-label").remove();
    container.selectAll(".line-axis").remove();
    container.selectAll(".legend-line").remove();
    container.selectAll(".axis-title").remove();

    const line = d3.line()
      .x(d => xScaleStep(d.step))
      .y(d => yScale(d.value));

    const hlKeys: (keyof typeof highLevelColors)[] = ["PLANNING", "IMPLEMENTATION", "REVISION"];

    hlKeys.forEach(hl => {
      const values = stepCounts.map(d => ({ step: d.step, value: d[hl] }));
      container.append("path")
        .datum(values)
        .attr("class", "line-axis")
        .attr("fill", "none")
        .attr("stroke", highLevelColors[hl])
        .attr("stroke-width", 2)
        .attr("d", line);
    });
    ...
    container.append("text")
      ...
      .attr("y", config.height - config.margin.bottom + offset - 20)
      .style("font-size", "13px")
      .style("font-weight", "bold")
      .style("fill", "#333")
      .text("High-Level Label Count Over Steps");

    const xAxis = d3.axisBottom(xScaleStep).ticks(Math.min(10, maxStep)).tickFormat(d => `Step ${d}`);
    container.append("g")
      .attr("class", "x-axis")
      .attr("transform", `translate(0,${config.height - config.margin.bottom + chartHeight + offset})`)
      .call(xAxis)
      .selectAll("text")
    ...
    brush = d3.brushX()
      .extent([
        [config.margin.left, config.height - config.margin.bottom + offset],
        [config.margin.left + innerWidth, config.height - config.margin.bottom + chartHeight + offset]
      ])
      .on("brush end", handleBrush);
    ...
    if (brushStepRange) {
      ...
      const x0 = xScaleStep(step0);
      const x1 = xScaleStep(step1);
      container.select(".brush").call(brush.move, [x0, x1]);
    }

    const legend = container.append("g").attr("class", "legend-line");
    const legendX = config.margin.left + innerWidth + 30;
    const legendYStart = config.height - config.margin.bottom + offset;

    hlKeys.forEach((hl, i) => {
      const yPos = legendYStart + i * 20;
      legend.append("rect")
        .attr("x", legendX)
        .attr("y", yPos)
        .attr("width", 10)
        .attr("height", 10)
        .attr("fill", highLevelColors[hl]);
...
<h1 style="margin-top: 80px;">Label Transition Flow Visualization</h1>
